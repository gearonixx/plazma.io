...
namespace real_medium::handlers::playlists::items_list {
...
std::string Handler::HandleRequest(
    ...
    int limit = pl::kDefaultItemsLimit;
    const auto limit_str = request.GetArg("limit");
    if (!limit_str.empty()) {
        try {
            limit = std::clamp(std::stoi(limit_str), 1, pl::kMaxItemsLimit);
    ...
    std::optional<pl::ItemCursor> cursor;
    const auto cursor_str = request.GetArg("cursor");
    if (!cursor_str.empty()) {
        cursor = pl::DecodeItemCursor(cursor_str);
        if (!cursor.has_value()) {
            request.SetResponseStatus(userver::server::http::HttpStatus::kBadRequest);
    ...
    try {
        ...
        // Build a single-partition query. The clustering composite is
        // (added_at ASC, video_id ASC); after a cursor the next page is
        //   added_at > k.added_at
        //   OR (added_at = k.added_at AND video_id > k.video_id)
        std::vector<pl::ItemSnapshot> items;
        items.reserve(limit + 1);
        bool has_more = false;
        userver::storages::scylla::Rows rows;
        if (cursor.has_value()) {
            rows = session_->Execute(
                ...
                "AND (added_at, video_id) > (?, ?) "
                "ORDER BY added_at ASC, video_id ASC LIMIT ?",
                std::vector<Value>{
                    Value{playlist_id},
                    Value{cursor->added_at_ms},
                    Value{cursor->video_id},
                    Value{static_cast<int32_t>(limit + 1)}
                }
            );
        } else {
            rows = session_->Execute(
        ...
        LOG_INFO() << "GET /v1/playlists/" << playlist_id << "/items user_id=" << auth.user_id
                   << " items=" << items.size() << " more=" << has_more;
        return userver::formats::json::ToString(response.ExtractValue());

    } catch (const std::exception& ex) {
        LOG_ERROR() << "GET /v1/playlists/" << playlist_id << "/items failed: " << ex.what();
